...
const OrderTotalSummary: React.FC<OrderTotalSummaryProps> = (props: any) => {
  ...
  const { subtotal, tax, shipping, shippingTax, totalDiscounts, grandTotal } =
  ...
  function initOrderTotalSummary() {
    ...
    if (order) {
      try {
        subtotal = order.totalProductPrice
        ...
        shipping = order.totalShippingCharge
          ? parseFloat(order.totalShippingCharge)
          : null;
        shippingTax = order.totalShippingTax
          ? parseFloat(order.totalShippingTax)
          : null;
        totalDiscounts = order.totalAdjustment
    ...
    <StyledGrid container>
      ...
      {tax != null && (
        <>
          <StyledGrid item xs={6}>
            <StyledTypography gutterBottom>
              {t("OrderTotalSummary.Labels.Tax")}
            </StyledTypography>
          </StyledGrid>
          <StyledGrid item xs={6}>
            <StyledTypography gutterBottom align="right">
      ...
      {shippingTax != null && (
        <>
          <StyledGrid item xs={6}>
            <StyledTypography gutterBottom>
              {t("OrderTotalSummary.Labels.ShippingTax")}
            </StyledTypography>
          </StyledGrid>
          <StyledGrid item xs={6}>
            <StyledTypography gutterBottom align="right">
